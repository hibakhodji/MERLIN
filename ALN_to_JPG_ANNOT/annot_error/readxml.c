...
/* this is the start of a new sequence, so reset the data for this sequence */
	...
	else if(strcmp(Element,"existence")==0) {
		Maln.seqs[Fseq+Nseqs].existence=atof(Content);
	}
		...
		if(strcmp(Content,"DOMAIN")==0) 
			...
			Ftype=DISORDER;
		else if (strcmp(Content,"KOANNO")==0) 
			Ftype=KOANNO;
			...
			Ftype=VARIANT;
		else if (strcmp(Content,"CONS_RES")==0) 
			Ftype=CONSRES;
		else if (strcmp(Content,"ANCHOR")==0) 
			Ftype=ANCHOR;
		else if (strcmp(Content,"BLOCK")==0) 
			Ftype=COREBLOCK;
		else if (strcmp(Content,"X0Motifs")==0) 
			Ftype=X0MOTIF;
		else if (strcmp(Content,"X1Motifs")==0) 
			Ftype=X1MOTIF;
		else if (strcmp(Content,"X1Motifs")==0) 
			Ftype=X1MOTIF;
			...
			Ftype=OTHER;
		if(Nftable[Ftype]<MAXFT-1) Nftable[Ftype]++;
		...
		if(Nftable[Ftype]<MAXFT) Maln.ft[Fseq+Nseqs].data[Ftype][Nftable[Ftype]].score=atof(Content);
                ...
                if(Nftable[Ftype]<MAXFT)
                        if(Content!=NULL) {
                                for(i=0;i<strlen(Content) && i<50;i++)
                                        if (!isspace(Content[i])) break;
				if(strlen(Content)>50) Content[49]='\0';
