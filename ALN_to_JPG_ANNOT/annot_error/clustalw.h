...
#define MAXNAMES		100	/* Max chars read for seq. names */
...
typedef struct {
	...
	sint existence;		/* type of evidence that supports the existence of the protein */
				/* 1. protein level 2. transcript level 3. Inferred from homology 4. Predicted 5. Uncertain */
...
typedef struct {
	char filename[FILENAMELEN+1];	/* input sequence file name */
	char treename[FILENAMELEN+1];	/* input phylogenetic file name */
	char alphabet[NUMRES+1];	/* the alphabet */
...
/* dsc.c */
int dsc(ALNPTR mult_aln, sint i, char *ss,float prob_a,float prob_b);

/* jnet.c */
int jnet(ALNPTR mult_aln, sint i, char *ss,float prob_a,float prob_b);
...
/* IUPRED */
int iupred(char *in_seq, char *strtype, int *glob_start, int *glob_end);
