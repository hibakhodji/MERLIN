...
static char * get_seq(char *sname,sint *len,char *nid,char *access,char *tit,sint *group,sint *seq_type,char *org,char *strand,Boolean dnaflag,sint *exist,FTPTR ft);
static char * get_clustal_seq(char *sname,sint *len,sint seqno);
static char * get_phylip_seq(char *sname,sint *len,sint seqno);
...
static sint count_phylip_seqs(void);
...
static char * get_phylip_seq(char *sname,sint *len,sint seqno)
/* read the seqno_th. sequence from a phylip multiple alignment file */
{
	static char line[MAXLINE+1];
	static char tseq[MAXLINE+1];
	static char tseq1[MAXLINE+1];
	char *seq = NULL;
	sint i,j;
	unsigned char c;
	Boolean first;
...
static char * get_seq(char *sname,sint *len,char *nid,char *access,char *tit,sint *group,sint *seq_type,char *org,char *strand,Boolean dnaflag,sint *exist,FTPTR ft)
{
	...
	char *seq = NULL;
	sint i, j, l, nft, ti, oi, g, offset = 0;
	sint i1,i2;
        unsigned char c=EOS;
		...
		case RSF:
			...
			while( !keyword(line,"sequence") ) {
				if (keyword(line,"descrip") ) {
					for(i=8;i<strlen(line)-1 && i<MAXTITLES;i++)
						tit[i-8]=line[i];
					tit[i-8]='\0';
				}
						...
						if(check_ft_type(feature,ft_name,&type)==1) {
							...
							if(ft_start>0 && ft_end>0) {
								if(nft>=MAXFT) {
									...
									if (fgets(line,MAXLINE+1,fin) == NULL) return NULL;
									sscanf(line,"%s%s%s%s%s",tmp1,tmp2,tmp3,tmp4,ft_name);
/*
									for(i=31;line[i]!='\n' && line[i]!='\0';i++) 
										ft_name[i-31]=line[i];
									ft_name[i-31]='\0';
...
leaving the data for profile 1 intact */
	...
	if (seqFormat == RELACS) {
		(*mult_aln)=read_xml(fin,first_seq);
		if(explicit_type==1)
			dnaflag1 = TRUE;
		else
			dnaflag1 = FALSE;
		...
		for(i=first_seq;i<first_seq+no_seqs;i++) {    /* get the seqs now*/
			...
			if(seqFormat == CLUSTAL) {
			...
			else if(seqFormat == PHYLIP) {
				seq1=get_phylip_seq(sname1,&l1,i-first_seq);
				strcpy(nid,sname1);
				strcpy(access,sname1);
			}
				...
				seq1=get_seq(sname1,&l1,nid,access,title,&group,&seq_type,organism,strand,mult_aln->dnaflag,&existence,&mult_aln->ft[i]);
...
/*  Set max length of dynamically allocated arrays in prfalign.c */
			...
			mult_aln->seqs[i].simgroup=group;
			/*mult_aln->seqs[i].existence=existence;*/
			if(explicit_type==0) {
...
static sint count_phylip_seqs(void)
/* count the number of sequences in a phylip alignment file */
{
	char line[MAXLINE+1];
	sint  ns,nl,nseqs;

	if (fgets(line,MAXLINE+1,fin) != NULL) {	/* get first line */
		/*if(sscanf(line,"%d %d",&ns,&nl) != 2) {
			return (sint)0;;
		}*/
	}						
	nseqs=0;
