...
void free_aln(ALNPTR mult_aln)
{
	...
	if(mult_aln->nseqs<=0) return;
		...
		ckfree(mult_aln->seqs[i].access);
		ckfree(mult_aln->seqs[i].nid);
...
sint check_ft_type(char *ft_type,char *ft_name,sint *type)
{
        ...
        else if (strcmp(ft_type,"CONS_RES")==0) {
                (*type)=CONSRES;
                ret=1;
        }
        ...
        else if (strcmp(ft_type,"KOANNO")==0) {
                (*type)=KOANNO;
                ret=1;
        }
        return ret;
...
void add_ft_entry(ALNPTR mult_aln,sint seq,sint first,sint last,sint type,sint code,float score,char *ctype,char *iname,sint is,sint ie)
{
	...
	if(last<first) return;
        ...
        mult_aln->ft[seq].data[type][n].start_col=fc;
        mult_aln->ft[seq].data[type][n].end_col=lc;
        if(fc==first)
	...
	if(strcmp(ctype,"BLOCK")==0) {
	...
	else if(strcmp(ctype,"PHYLOBLOCK")==0) {
		color=code;
	}
        ...
        else if(strcmp(ctype,"ELM")==0) {
                if(strncmp(name,"PRED_",5)==0) color=LGRAY;
                else if(strncmp(name,"PROP_",5)==0) color=GRAY;
                else color=code;
        }
        else if(strcmp(ctype,"DISORDER")==0) {
                if(strncmp(name,"PRED_",5)==0) color=LGRAY;
                else if(strncmp(name,"PROP_",5)==0) color=GRAY;
                else color=code;
        }
...
/* check for orphan sequences in the group */
		...
		for(j=0;j<groups[i].len;j++) {
			...
			/*if(meanid<0.2) orphans[j]=TRUE;*/
			if(meanid<0.30) orphans[j]=TRUE;
	...
	if(groups[ngroups-1].len>0) {
		...
		for(j=0;j<groups[ngroups].len;j++) {
			...
			if(secgroup[orphans[i]]==0) {
				...
				/*if(j!=orphans[i] && tmat[orphans[i]][j]<0.5) {*/
				if(j!=orphans[i] && tmat[orphans[i]][j]<0.75) {
					if(secgroup[j]!=0) {
						n=groups[secgroup[j]-1].len;
						groups[secgroup[j]-1].seqs[n]=orphans[i];
						groups[secgroup[j]-1].len=n+1;
						secgroup[orphans[i]]=secgroup[j];
                				mult_aln.seqs[orphans[i]].simgroup=secgroup[j];
					}
					else {
        					groups[ngroups].seqs=(sint *)ckalloc((mult_aln.nseqs+1) * sizeof(sint));
						groups[ngroups].seqs[0]=orphans[i];
						groups[ngroups].seqs[1]=j;
						groups[ngroups].len=2;
						secgroup[orphans[i]]=ngroups+1;
                				mult_aln.seqs[orphans[i]].simgroup=ngroups+1;
						secgroup[j]=ngroups+1;
                				mult_aln.seqs[j].simgroup=ngroups+1;
						ngroups++;
					}
					break;
				}
