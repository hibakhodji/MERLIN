...
void relacs_out(FILE *rout, sint fres, sint len, ALN mult_aln, char *aln_name)
{
	...
	fprintf(rout,"<?xml version=\"1.0\"?>\n");
 	fprintf(rout,"<!DOCTYPE macsim SYSTEM \"http://lbgi.fr/macsim.dtd\">\n");
 	fprintf(rout,"<macsim>\n");
	...
	if(mult_aln.validalnscore)
 		fprintf(rout,"<aln-score>%.2f</aln-score>\n",mult_aln.alnscore);
...
/* decide the line length for this alignment - maximum is LINELENGTH */
	...
	for(ii=0; ii<nseqs; ii++) {
		...
		fprintf(rout, "<existence>%d</existence>\n",mult_aln.seqs[i].existence);
		...
		slen = rlen = 0;
		for(j=fres; j<fres+len && j<mult_aln.seqs[i].len; j++) {
			val = mult_aln.seqs[i].data[j];
			if(val==EOS)
				break;
			else if(!isalpha(val))
