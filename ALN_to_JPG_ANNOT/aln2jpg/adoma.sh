...
# Don't do the alignment, just change the file format : Julie
#/biolo/clustal/bin/clustalw -infile=$file_path"$PREFIX".fa -convert -type="$TYPE"  &>>"$file_path"logfile.txt
$scriptpath/convseq/convseq $file_path"$PREFIX".fa $file_path"$PREFIX".aln
echo
